defmodule TelemetricEvents.Logger.JSONBackend do
  ...
  def handle_event(:flush, state), do: {:ok, flush(state)}
  ...
  defp await_io(%{ref: ref} = state) do
    receive do
      ...
      {:io_reply, ^ref, error} ->
        error |> handle_io_reply(state) |> await_io()
  ...
  defp format_event(level, msg, ts, md, state) do
    %{metadata: keys, colors: colors} = state

    level
    |> JSONFormatter.format(msg, ts, take_metadata(md, keys))
  ...
  defp handle_io_reply({:error, error}, _) do
    raise "failure while logging console message: " <> inspect(error)
  end
  ...
  defp flush(state), do: state |> await_io() |> flush()
end
